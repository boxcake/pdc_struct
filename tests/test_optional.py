...
def test_all_optional_model():
    ...
    assert empty_data[1] & HeaderFlags.HAS_OPTIONAL_FIELDS != 0, f"Optional fields flag not set: {bin(empty_data[1])}"
    ...
    assert empty_data[4] == 1, f"Wrong bitmap length: {empty_data[4]}"  # 1 byte needed for bitmap
    assert empty_data[5] == 0, f"Bitmap not empty: {bin(empty_data[5])}"  # All bits should be 0

    empty_recovered = AllOptionalModel.from_bytes(empty_data)
    assert all(getattr(empty_recovered, f) is None
    ...
    assert all(getattr(empty_recovered, f) is None for f in ['field1', 'field2', 'field3'])
...
def test_optional_fields_c_mode():
    with pytest.raises(ValueError,match="Optional fields in C_COMPATIBLE mode must have either a default value or default_factory"):
        class OptionalFieldsModelNoDefault(StructModel):
            field1: Optional[int] = Field(None, description="Optional field - no default")
            struct_config = StructConfig(
                mode=StructMode.C_COMPATIBLE,
            )

    class OptionalFieldsModelDefault(StructModel):
        field1: Optional[int] = Field(default=100, description="Optional field - with default")
        struct_config = StructConfig(
            mode=StructMode.C_COMPATIBLE,
        )
    ...
    class OptionalFieldsModelFactory(StructModel):
        field1: Optional[int] = Field(default_factory=lambda: int(99), description="Optional field - with factory")
        struct_config = StructConfig(
            mode=StructMode.C_COMPATIBLE,
        )
